...
EOS Version:
    -Typically kept up-to-date with the most recent release of the normal PicCRDS python program
    -Added functionality with EOS-specific files:
    -Adds rain, temperature, and RH data to the output _analysis csv
    -Rain, temp, RH data comes from NTU.climate.data_STARTDATE_ENDDATE.xlsx
    -This file is created in R with new.climate.data.aggregation.R 
        -Curently found in ~/R/bin
...
dir_files = os.listdir(home+'/Documents/EOS/data/HOBO_gauge_data/new.format')
#keep only the xlsx file
#the file might exist with a ~ appended if the file is currently opened, so have to remove that files as well
climate_data = ''.join([x for x in dir_files if '.csv' in x and 'NTU' in x])
#read in climate data
climate_data = pd.read_csv(home+'/Documents/EOS/data/HOBO_gauge_data/new.format/'+climate_data)
